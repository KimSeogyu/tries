...
func (t *MerklePatriciaTrie) Search(key []byte) ([]byte, bool) {
	node := t.root
	...
	for {
		...
		if branch, ok := node.(*BranchNode); ok {
			...
			brches, remaining := nibbles[0], nibbles[1:]
			nibbles = remaining
			node = branch.Branches[brches]
			continue
		}
...
func (t *MerklePatriciaTrie) Put(key []byte, value []byte) {
